...
class TaskState:
    ...
    caller_context: Context | None = None
                ...
                while not shutdown_event.is_set():
                    try:
                        ...
                        try:
                            ...
                            task_state.execution_task = task_state.caller_context.run(
                                asyncio.create_task, func(*args, **kwargs)
                            )
                            if max_execution_timeout is not None:
                                result = await asyncio.wait_for(
                                    task_state.execution_task, timeout=max_execution_timeout
                                )
                            else:
                                result = await task_state.execution_task
                        ...
                        for task_id, execution_duration in stuck_tasks:
                            ...
                            async with task_states_lock:
                                if task_id in task_states:
                                    ...
                                    if task_state.execution_task is not None:
                                        task_state.execution_task.cancel()
                                    task_states.pop(task_id, None)
        ...
        async def wait_func(
            *args, _priority=10, _timeout=None, _queue_timeout=None, **kwargs
            ...
            try:
                ...
                async with task_states_lock:
                    task_states.pop(task_id, None)
                if task_state.execution_task is not None and not task_state.execution_task.done():
                    task_state.execution_task.cancel()
                    await asyncio.gather(task_state.execution_task, return_exceptions=True)
