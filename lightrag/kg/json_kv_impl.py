...
class JsonKVStorage(BaseKVStorage):
    ...
    async def delete_query_cache(self) -> None:
        """Invalidate generated answers after document changes; retain extraction caches."""
        async with self._storage_lock:
            identifiers = [key for key in self._data if len(key.split(':')) == 3
                           and key.split(':')[1] == 'query']
        if identifiers:
            await self.delete(identifiers)
